...
fn drag_and_drop_item(
    ...
    let mouse_position = cursor_moved_events
        .iter()
        .last()
        .map(|cursor| cursor.position - Vec2::new(window.width(), window.height()) / 2.)
    ...
    *last_mouse_position = mouse_position;
    ...
    if mouse.just_pressed(MouseButton::Left) {
        grabbed_item.0 = items
            ...
            .find(|(_, transform, collider)| {
                collider.contains_local_point(mouse_position - transform.translation.truncate())
            })
            .map(|(entity, ..)| entity);
    }
    ...
    if let Some(item) = grabbed_item.0 {
        ...
        transform.translation = Vec3::from((mouse_position, Z));
    }
}
